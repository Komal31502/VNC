...
const Performance = ({spreadsheet}) => {
    ...
    return (
        ...
        <div className='header-titles'>
            <div className='title-style'>BSC Performance Spreadsheet</div>
                    ...
                    <TableContainer component={Paper} sx={{
                        ...
                        <Table stickyHeader aria-label="collapsible table" sx={{ height: "100%" }}>
                                ...
                                <TableRow hover={false}>
                                    {
                                        PerformanceComponents.map((per, index) => (
                                            <TableCell align={per === "Performance" && index === 0 ? 
                                            "left":
                                            index === PerformanceComponents.length-1 ? "right"
                                            : "center"
                            ...
                            <TableBody>
                                ...
                                >
                                    <TableCell align="left" size="large" sx={{ color: "var(--text-primary, #212529)", fontFamily: "Figtree", fontSize: "16px", fontWeight: "600", lineHeight: "150%" }}>
                                        <img src="/images/scoreStrategyMap.svg" alt="" style={{ width: "24px", height: "24px" }} /> <span style={{ paddingLeft: "8px" }}>{tableData?.name}</span>
                                    </TableCell>
                                    <TableCell></TableCell>
                                    <TableCell 
                                        align="center">{tableData?.measure}</TableCell>
                                    <TableCell  align="center">{tableData?.value}</TableCell>
                                    <TableCell  align="center">{tableData?.weight}</TableCell>
                                    <TableCell  align="center">{tableData?.max}</TableCell>
                                    <TableCell  align="center">{tableData?.min}</TableCell>
                                    <TableCell  align="right">{tableData?.performance}</TableCell>
                                </TableRow>
                                {
                                    tableData?.perspectives?.map((per, index) => (
                                        <React.Fragment>
                                            ...
                                            selected={type.name === "perspective" && type.id === per.id}
                                                ...
                                                sx={{color: "var(--text-primary, #212529)", 
                                                ...
                                                <TableCell align="center" sx={{ width: "10%" }} onDoubleClick={() => setEdit((prev) => ({ ...prev, index: index, item: true, ind: 1 }))} onClick={() => setEdit((prev) => ({ ...prev, index: index, item: false, ind: 1 }))}>

                                                    {per.measure}

                                                </TableCell>
                                                <TableCell align="center" sx={{ width: "10%" }} onDoubleClick={() => setEdit((prev) => ({ ...prev, index: index, item: true, ind: 2 }))} onClick={() => setEdit((prev) => ({ ...prev, index: index, item: false, ind: 2 }))}>
                                                    {per.measure}
                                                </TableCell>
                                                <TableCell align="center" sx={{ width: "10%" }} onDoubleClick={() => setEdit((prev) => ({ ...prev, index: index, item: true, ind: 3 }))} onClick={() => setEdit((prev) => ({ ...prev, index: index, item: false, ind: 3 }))}>
                                                    {per.weight}
                                                </TableCell>
                                                <TableCell align="center" sx={{ width: "10%" }} onDoubleClick={() => setEdit((prev) => ({ ...prev, index: index, item: true, ind: 4 }))} onClick={() => setEdit((prev) => ({ ...prev, index: index, item: false, ind: 4 }))}>
                                                    {"_"}
                                                </TableCell>
                                                <TableCell align="center" sx={{ width: "10%" }} onDoubleClick={() => setEdit((prev) => ({ ...prev, index: index, item: true, ind: 5 }))} onClick={() => setEdit((prev) => ({ ...prev, index: index, item: false, ind: 5 }))}>
                                                    {"_"}
                                                </TableCell>
                                                <TableCell align="right" sx={{ width: "10%" }} onDoubleClick={() => setEdit((prev) => ({ ...prev, index: index, item: true, ind: 6 }))} onClick={() => setEdit((prev) => ({ ...prev, index: index, item: false, ind: 6 }))}>
                                                    {
                                                        per.performance? per.performance : "_"
                                                    }
                                                </TableCell>
                                            </TableRow>
                                            <TableRow>
